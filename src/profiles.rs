use crate::auth::{complete_device_login, load_accounts, request_device_code, save_accounts, MinecraftAccount};
use gtk4::prelude::*;
...
pub fn open_profiles_dialog(parent: Option<&gtk4::Window>) {
    ...
    let accounts_box_clone = accounts_box.clone();
    let status_clone = status.clone();
    let login_clone = login_button.clone();
    let rt = std::sync::Arc::new(tokio::runtime::Runtime::new().expect("tokio runtime"));
    let rt_load = rt.clone();
    rt_load.spawn(async move {
        ...
        let _ = glib::MainContext::default().spawn_local(async move {
            while let Some(child) = accounts_box_clone.first_child() { accounts_box_clone.remove(&child); }
            if names.is_empty() {
                accounts_box_clone.append(&Label::new(Some("No Microsoft accounts added yet.")));
            } else {
                for name in names {
                    let row = Label::builder().label(&format!("✓  {}", name)).halign(Align::Start).build();
                    accounts_box_clone.append(&row);
                }
            }
        });
    });
    ...
    login_button.connect_clicked(move |_| {
        login_clone.set_sensitive(false);
        status_clone.set_label("Requesting a Microsoft sign-in code…");

        let status_device = status_clone.clone();
        let button_device = login_clone.clone();
        let accounts_box_device = accounts_box.clone();
            ...
            match request_device_code().await {
                Ok(device) => {
                    ...
                    let _ = glib::MainContext::default().spawn_local(async move {
                        match result {
                            Ok(account) => {
                                let mut accounts = load_accounts(&path_login).await.unwrap_or_default();
                                accounts.retain(|a| a.id != account.id);
                                accounts.push(account.clone());
                                if let Err(error) = save_accounts(&path_login, &accounts).await {
                                    status_device.set_label(&format!("Login succeeded, but saving the account failed: {}", error));
                                } else {
                                    status_device.set_label(&format!("Signed in as {}.", account.name));
                                    let row = Label::builder().label(&format!("✓  {}", account.name)).halign(Align::Start).build();
                                    accounts_box_device.append(&row);
                                }
                            }
                            Err(error) => status_device.set_label(&format!("Microsoft login failed: {}", error)),
                        }
                        button_device.set_sensitive(true);
                    });
                }
                Err(error) => {
                    let _ = glib::MainContext::default().spawn_local(async move {
                        status_device.set_label(&format!("Could not start Microsoft login: {}", error));
                        button_device.set_sensitive(true);
                    });
                }
            }
