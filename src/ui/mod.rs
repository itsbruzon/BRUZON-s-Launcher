...
impl SimpleComponent for AppModel {
    ...
    fn init(
        ...
        back_button.connect_clicked(move |_| sender_clone.input(AppMsg::BackToMainMenu));
        ...
        let sidebar_toggle_button = gtk::Button::builder()
            ...
            .build();
        let sender_clone = sender.clone();
        sidebar_toggle_button.connect_clicked(move |_| sender_clone.input(AppMsg::ToggleSidebar));
    ...
    fn update(&mut self, msg: Self::Input, sender: ComponentSender<Self>) {
        match msg {
            ...
            AppMsg::VersionsLoaded(result) => match result {
                Ok(versions) => {
                    ...
                    if let Some(string_list) = &self.version_list_model {
                        string_list.splice(0, string_list.n_items(), &self.sorted_versions.iter().map(String::as_str).collect::<Vec<_>>());
                    }
                }
                Err(e) => self.error_message = Some(format!("Failed to load versions: {}", e)),
            },
            AppMsg::ProfilesLoaded(result) => match result {
                Ok(profiles) => self.profiles = profiles,
                Err(e) => self.error_message = Some(format!("Failed to load profiles: {}", e)),
            },
                            ...
                            match launcher_clone.prepare_and_launch(
                                ...
                                Ok(mut command) => match command.spawn() {
                                    Ok(mut child) => {
                                        ...
                                        sender_clone.input(AppMsg::SessionEnded(profile_name_clone, start_time.elapsed().as_secs()));
                                        sender_clone.input(AppMsg::LaunchCompleted);
                                    }
                                    Err(e) => sender_clone.input(AppMsg::Error(format!("Failed to spawn: {}", e))),
                                },
                                Err(e) => sender_clone.input(AppMsg::Error(format!("Launch Failed: {}", e))),
                            }
                        });
                    }
                }
            ...
            AppMsg::LaunchCompleted => self.state = AppState::Ready { current_section: Section::Home },
            AppMsg::UsernameChanged(username) => self.input_username = username,
            AppMsg::RamChanged(ram) => self.input_ram = ram,
            AppMsg::VersionSelected(version) => {
                self.fabric_switch_enabled = crate::utils::is_at_least_1_14(&version);
                if !self.fabric_switch_enabled {
                    self.input_install_fabric = false;
                }
                self.input_version = Some(version);
            }
            AppMsg::ToggleFabric(install) => self.input_install_fabric = install,
            AppMsg::SaveProfile => {
                if self.input_username.trim().is_empty() || self.input_version.is_none() {
                    return;
                }
            ...
            AppMsg::BackToMainMenu => sender.input(AppMsg::NavigateToSection(Section::Home)),
            AppMsg::Error(message) => self.state = AppState::Error { message },
        ...
        match &self.state {
            ...
            AppState::Ready { current_section } => {
                ...
                match current_section {
                    ...
                    Section::Profiles => widgets.content_stack.set_visible_child_name("profiles"),
